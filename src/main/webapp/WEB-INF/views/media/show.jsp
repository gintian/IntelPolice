...
<div class="bjui-pageHeader">
  <form id="${searchFormId}" data-toggle="ajaxsearch" action="${root}<%=Url.getUrl(Url.MEDIA_PAGE_SHOW)%>" method="post">
    ...
    <c:forEach items="${list}" var="item">
      <tr>
        <td class="option" align="center">
          <sys:permission url="<%=Url.getUrl(Url.VIEWDOC_DETAIL_SHOW)%>">
            <a href="${root}<%=Url.getUrl(Url.VIEWDOC_DETAIL_SHOW)%>?docId=${item.docId}" class="btn btn-green"
               data-toggle="dialog" data-mask="true" data-width="600" data-height="500" data-confirm-msg="详情">详情</a>
          </sys:permission>
          <sys:permission url="<%=Url.getUrl(Url.VIEWDOC_FILE_SHOW)%>">
            <a href="${root}<%=Url.getUrl(Url.VIEWDOC_FILE_SHOW%>?docId=${item.docId}" class="btn btn-green"
...
<%--          <sys:permission url="<%=Url.getUrl(Url.MEDIA_CASE_SHOW)%>">
            <a href="${root}<%=Url.getUrl(Url.MEDIA_CASE_SHOW)%>?docId=${item.docId}" class="btn btn-blue"
