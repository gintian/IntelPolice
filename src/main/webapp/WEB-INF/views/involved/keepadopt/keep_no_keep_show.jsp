                ...
                <div id="layout-01" style="height:94%; overflow:hidden;">
                    <div class="bjui-pageHeader">
                        <form id="${searchFormId}" data-toggle="ajaxsearch"
                              action="${root}<%=Url.getUrl(Url.INVOLVED_KEEP_NO_KEEP_SHOW)%>"
                              method="post">
                    ...
                    <table class="table table-bordered table-hover table-striped table-top data_table table_center"
                        ...
                        <c:forEach items="${list}" var="item">
                            <tr>
                                <td class="option">
                                    <sys:permission url="<%=Url.getUrl(Url.INVOLVED_KEEP_TASK_KEEP_SHOW)%>">
                                        <%--<a href="${root}<%=Url.getUrl(Url.INVOLVED_KEEP_TASK_KEEP_SHOW)%>?taskId=${item.taskId}&remark=${item.remark}"--%>
                                           <%--class="btn btn-blue" data-toggle="dialog" data-mask="true" data-id="rkdialog"--%>
                                           <%--data-width="600" data-height="500" data-confirm-msg="处理">处理</a>--%>
                                        <a href="${root}<%=Url.getUrl(Url.INVOLVED_KEEP_TASK_KEEP_SHOW)%>?taskId=${item.taskId}&remark=${item.remark}"
                                           class="btn btn-blue" data-toggle="navtab" data-id="navtab"
                                           data-title="财物入库">处理</a>
                                    </sys:permission>
                                ...
                                <td title="${item.identityCard}"><hs:userByIdentityCard
                                        value="${item.identityCard}"/></td>
