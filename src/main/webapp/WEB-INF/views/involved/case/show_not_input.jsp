...
<div class="bjui-pageHeader">
    <form id="${searchFormId}" data-toggle="ajaxsearch" action="${root}<%=Url.getUrl(Url.INVOLVED_CASE_NOT_INPUT)%>" method="post">
        <div class="bjui-searchBar">
            ...
            <select id="caseclass" data-toggle="selectpicker" name="caseclass" data-val="${searchObj.caseclass}" data-width="200">
                ...
                <c:forEach var="item" items="${codeBeanList}">
                    <c:choose>
                        <c:when test="${item.code == searchObj.caseclass}">
                            <option value="${item.code}" selected>${item.codedesc}</option>
                        </c:when>
                        <c:otherwise>
        ...
        <c:forEach items="${list}" var="item">
            <tr>
                <td align="center">
                    ...
                    <sys:permission url="<%=Url.getUrl(Url.INVOLVED_CASE_REQUIRE_SYNC)%>">
                        <a href="${root}<%=Url.getUrl(Url.INVOLVED_CASE_REQUIRE_SYNC)%>?caseid=${item.caseid}" class="btn btn-orange" data-toggle="doajax" data-confirm-msg="确定要需要同步吗？">需要同步</a>
                    </sys:permission>
                </td>
