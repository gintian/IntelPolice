                    ...
                    <c:if test="${empty entity}" var="flag">
                        <div class='btn lattice-content lattice-btn' id='box-${ i.index * fn:length(boxnum) + j.count }' >
                            ...
                            <div class='lattice-content1'>
                                <sys:permission url="<%=Url.getUrl(Url.ARCHIVE_STORES_BOX_ADD_SHOW)%>">
                                    <a href='${root}<%=Url.getUrl(Url.ARCHIVE_STORES_BOX_ADD_SHOW)%>?storesId=${requestScope.archiveStores.lockersId}&code=${i.index * fn:length(boxnum) + j.count }' data-toggle='navtab' data-id='navtab' data-title='新增箱子' class='btn btn-warning btn-type1'>新增</a>
                                </sys:permission>
                            </div>
                        </div>
                    </c:if>
                        ...
                        <div class='btn btn-primary lattice-content lattice-btn' id='box-${ i.index * fn:length(boxnum) + j.count }' style="background:rgba(255,255,255,0);" >
                            ...
                            <div class='lattice-content1'>
                                <c:if test="${state eq 0}" var="flag1">
                                    <sys:permission url="<%=Url.getUrl(Url.ARCHIVE_STORES_BOX_DETAIL_SHOW)%>">
                                        <a href='${root}<%=Url.getUrl(Url.ARCHIVE_STORES_BOX_DETAIL_SHOW)%>?id=${entity.id}' data-toggle='dialog' data-mask='true' data-width='600' data-height='500' class='btn btn-info btn-type1'>查看</a><br>
                                    </sys:permission>
                                </c:if>
                                <c:if test="${not flag1 and isBoxUser[entity.code] eq 1}">
                                    <sys:permission url="<%=Url.getUrl(Url.ARCHIVE_STORES_BOX_DETAIL_SHOW)%>">
                                        <a href='${root}<%=Url.getUrl(Url.ARCHIVE_STORES_BOX_DETAIL_SHOW)%>?id=${entity.id}' data-toggle='dialog' data-mask='true' data-width='600' data-height='500' class='btn btn-info btn-type1'>查看</a><br>
                                    </sys:permission>
                                </c:if>
                                <sys:permission url="<%=Url.getUrl(Url.ARCHIVE_STORES_BOX_UPDATE_SHOW)%>">
                                    <a href='${root}<%=Url.getUrl(Url.ARCHIVE_STORES_BOX_UPDATE_SHOW)%>?id=${entity.id}' class='btn btn-default btn-type1' data-toggle='navtab' data-id='navtab' data-title='编辑'>编辑</a>
                                </sys:permission>
                            </div>
                        </div>
                    </c:if>
