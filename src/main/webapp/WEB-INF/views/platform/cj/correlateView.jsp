...
<div class="bjui-pageHeader">
    <form id="${searchFormId}" data-toggle="ajaxsearch"
          action="${root}<%=Url.getUrl(Url.CJ_PAGE_SHOW)%>" method="post">
        ...
        <c:forEach items="${list}" var="item">
            <tr>
                <td class="option" align="center">
                    <a href="${root}<%=Url.getUrl(Url.CJ_DETAIL_SHOW)%>?cjdbh=${item.cjdbh}"
                       class="btn btn-green" data-toggle="dialog" data-mask="true"
                       data-width="800" data-height="500">处警详情</a>
                    <a href="${root}<%=Url.getUrl(Url.VIEWDOC_ONE_VIEWS_SHOW)%>?cjdbh=${item.cjdbh}"
                       class="btn btn-green" data-toggle="navtab" data-mask="true"
                       data-width="800" data-height="500" >文件详情</a>
                    <a href="${root}<%=Url.getUrl(Url.VIEWDOC_CORRELATE_SHOW)%>?cjdbh=${item.cjdbh}"
                       class="btn btn-green" data-toggle="dialog" data-mask="true"
