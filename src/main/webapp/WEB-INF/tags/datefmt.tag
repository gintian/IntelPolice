...
<%
    ...
    request.setAttribute("dateTimeFormat1", DateUtils.DEFAULT_DAY_CHINESE_PATTERN);//yyyy年MM月dd日  HH时mm分
    Date date = null;
...
<c:choose>
    ...
    <c:otherwise>
        <c:choose>
            ...
            <c:when test="${type eq 'timeFormatDate'}">
                <fmt:formatDate value="${date}" pattern="${dateTimeFormat1}"/>
            </c:when>
            <c:otherwise>
