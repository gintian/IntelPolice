...
public class CriminalAction extends BaseAction<Criminal> implements ModelDriven<Criminal> {
    ...
    @Action(value = Url.RECORD_PAGE_SHOW_NO_INPUT, results = {
            @Result(name = "SUCCESS", location = Url.VIEWS + "record/show_archive.jsp"),
    ...
    public String showRecordNoInputPage() {
        try {
            ...
            /*for (CriminalBean criminal : entityList) {//processPersonName
                bpipUserBean = bpipUserService.findBpipUserById(criminal.getCaseid());
                if(bpipUserBean != null){
                    criminal.setProcessPersonName(bpipUserBean.getName());
                } else{
                    criminal.setProcessPersonName("--");
                }
     ...
     * @return
     */
    @Action(value = Url.RECORD_PAGE_SHOW_INPUT, results = {
            @Result(name = "SUCCESS", location = Url.VIEWS + "record/show_archive.jsp"),
            @Result(name = "ERROR", location = "../sys_error.jsp")})
    public String showRecordInputPage() {
        try {
            criminalService.findBeanPageForModule(getPageBean(), bean, CommonConstant.MODULE_RECORD_INPUT);
            List<CriminalBean> entityList = pageBean.getRows();
     ...
     * @throws Exception
     */
    @Action(value = Url.CRIMINAL_SMS_NOTCG_PAGE_SHOW, results = {
            @Result(name = "SUCCESS", location = Url.VIEWS + "case/smsLog/show.jsp")})
    public String indexSmsLogCg() throws Exception {
        // 获取当前登录的用户
        User user = this.getUser(request);
        // 查询数据
        criminalService.pageSmsLogQuery(getPageBean(), bean, "fs");
        // 获取分页数据
        List entityList = pageBean.getRows();
        // 填充查询对象
        request.setAttribute("searchObj", bean);
        //未成功or未发送or重复
        request.setAttribute("stuts", "wcg");
        // 填充分页对象
        this.handlePageData(entityList, pageBean, request);
        return "SUCCESS";
    }

    @Action(value = Url.CRIMINAL_SMS_NOTFS_PAGE_SHOW, results = {
            @Result(name = "SUCCESS", location = Url.VIEWS + "case/smsLog/show.jsp")})
    public String indexSmsLogFs() throws Exception {
        // 获取当前登录的用户
        User user = this.getUser(request);
        // 查询数据
        criminalService.pageSmsLogQuery(getPageBean(), bean, "wfs");
        // 获取分页数据
        List entityList = pageBean.getRows();
        // 填充查询对象
        request.setAttribute("searchObj", bean);
        //未成功or未发送or重复
        request.setAttribute("stuts", "wfs");
        // 填充分页对象
        this.handlePageData(entityList, pageBean, request);
        return "SUCCESS";
    }

    @Action(value = Url.CRIMINAL_SMS_CF_PAGE_SHOW, results = {
            @Result(name = "SUCCESS", location = Url.VIEWS + "case/smsLog/show.jsp")})
    public String indexSmsLogCf() throws Exception {
        // 获取当前登录的用户
        User user = this.getUser(request);
        // 查询数据
        criminalService.pageSmsLogQuery(getPageBean(), bean, "cf");
        // 获取分页数据
        List entityList = pageBean.getRows();
        // 填充查询对象
        request.setAttribute("searchObj", bean);
        //未成功or未发送or重复
        request.setAttribute("stuts", "cf");
        // 填充分页对象
        this.handlePageData(entityList, pageBean, request);
        return "SUCCESS";
    }
     ...
     * @throws Exception
     */
    @Action(value = Url.CRIMINAL_NOTJCJ_PAGE_SHOW, results = {
            @Result(name = "SUCCESS", location = Url.VIEWS + "case/criminal/notJcj/show.jsp")})
    public String indexNotJcj() throws Exception {
        // 获取当前登录的用户
        User user = this.getUser(request);
        // 查询数据
        criminalService.pageNotjcjQuery(getPageBean(), bean);
        // 获取分页数据
        List entityList = pageBean.getRows();
        // 填充查询对象
        request.setAttribute("searchObj", bean);
     ...
     * @throws Exception
     */
    @Action(value = Url.CRIMINAL_NOTJCJ_DETAIL_SHOW,
            results = {@Result(name = "SUCCESS", location = Url.VIEWS + "case/criminal/notJcj/detail.jsp")})
    public String getNotJcjDetails() throws Exception {
        try {
            CriminalBean bean2 = criminalService.findCriminalById(bean.getCaseid());
            request.setAttribute("entity", bean2);
        } catch (Exception e) {
     ...
     * @throws Exception
     */
    @Action(value = Url.CRIMINAL_ZJSL_PAGE_SHOW, results = {
            @Result(name = "SUCCESS", location = Url.VIEWS + "case/criminal/zjsl/show.jsp")})
    public String indexZjsl() throws Exception {
        // 获取当前登录的用户
        User user = this.getUser(request);
        // 查询数据
        criminalService.pageZjslQuery(getPageBean(), bean);
        // 获取分页数据
        List entityList = pageBean.getRows();
        // 填充查询对象
        request.setAttribute("searchObj", bean);
    ...
    //没关联音视频的案件
    @Action(value = Url.CRIMINAL_NOTCORRELATE_VIEW_SHOW, results = {
            @Result(name = "SUCCESS", location = Url.VIEWS + "case/criminal/zjsl/notCorrelateView.jsp")})
    public String selectCriminalNotView() {

        criminalService.pageQueryNotView(getPageBean(), bean);
    ...
    @Action(value = Url.CRIMINAL_CORRELATE_VIEW_SHOW, results = {
            @Result(name = "SUCCESS", location = Url.VIEWS + "case/criminal/zjsl/correlateView.jsp")})
    public String selectCriminalView() {
        criminalService.pageQueryCorrelateView(getPageBean(), bean);
        List list = pageBean.getRows();
        // 填充查询对象
        request.setAttribute("searchObj", bean);
        // 填充分页对象
