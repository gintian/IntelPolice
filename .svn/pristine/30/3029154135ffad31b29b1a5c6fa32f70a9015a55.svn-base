...
<div class="bjui-pageHeader">
    <form id="${searchFormId}" data-toggle="ajaxsearch"
          action="${root}<%=Url.getUrl(Url.INVESTIGATIONSUPALARM_NO_ACCEPT_PAGE_SHOW)%>" method="post">
...
<table class="table table-bordered table-hover table-striped table-top data_table table_center"
    ...
    <c:forEach items="${list}" var="item">
        <tr>
            <td class="option">
                <a href="${root}<%=Url.getUrl(Url.INVESTIGATIONSUPALARM_DETAIL_SHOW)%>?suspicId=${item.suspicId}"
            ...
            <td title="${item.keywords}"><hs:out value="${item.keywords}" length="15"/></td>
