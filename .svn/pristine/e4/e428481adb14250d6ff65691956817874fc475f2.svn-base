...
<div class="bjui-pageContent">
    <c:choose>
        <c:when test="${iden=='法制'}">
        <form id="${searchFormId}" action="${root}<%=Url.getUrl(Url.INVOLVED_KEEP_TASK_FZ_ADOPT)%>" data-toggle="validate"
              data-reload-navtab="true" method="post">
        </c:when>
        <c:when test="${iden=='领导'}">
            <form id="${searchFormId}" action="${root}<%=Url.getUrl(Url.INVOLVED_KEEP_TASK_LD_ADOPT)%>" data-toggle="validate"
                  data-reload-navtab="true" method="post">
        </c:when>
    </c:choose>
