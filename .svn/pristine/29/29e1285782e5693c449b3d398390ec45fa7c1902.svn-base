                ...
                <div id="layout-01" style="height:94%; overflow:hidden;">
                    <div class="bjui-pageHeader">
                        ...
                        <form id="${searchFormId}" data-toggle="ajaxsearch"
                              action="${root}<%=Url.getUrl(Url.INVOLVED_DISPOSE_FZ_NO_ADOPT_SHOW)%>"
                              method="post">
                            </c:when>
                            <c:when test="${iden==领导}">
                            <form id="${searchFormId}" data-toggle="ajaxsearch"
                                  action="${root}<%=Url.getUrl(Url.INVOLVED_DISPOSE_LD_NO_ADOPT_SHOW)%>"
                                  method="post">
                                </c:when>
                                </c:choose>
                                <div class="bjui-searchBar">
                                    ...
                                    <label for="caseId" class="control-label">案事件ID：</label>
                                    <input name="caseId" id="caseId" placeholder="请输入案事件ID" style="width: 200px;"
                        ...
                        <c:forEach items="${list}" var="item">
                            <tr>
                                <td class="option">
                                    <c:choose>
                                        <c:when test="${iden=='法制'}">
                                            <sys:permission url="<%=Url.getUrl(Url.INVOLVED_DISPOSE_TASK_FZ_ADOPT_SHOW)%>">
                                                <a href="${root}<%=Url.getUrl(Url.INVOLVED_DISPOSE_TASK_FZ_ADOPT_SHOW)%>?taskId=${item.taskId}&remark=${item.remark}"
                                                   class="btn btn-blue" data-toggle="dialog" data-mask="true"
                                                   data-width="600"
                                                   data-height="500" data-confirm-msg="处理">处理</a>
                                            </sys:permission>
                                        </c:when>
                                        <c:when test="${iden=='领导'}">
                                            <sys:permission url="<%=Url.getUrl(Url.INVOLVED_DISPOSE_TASK_LD_ADOPT_SHOW)%>">
                                                <a href="${root}<%=Url.getUrl(Url.INVOLVED_DISPOSE_TASK_LD_ADOPT_SHOW)%>?taskId=${item.taskId}&remark=${item.remark}"
                                                   class="btn btn-blue" data-toggle="dialog" data-mask="true"
                                                   data-width="600"
                                                   data-height="500" data-confirm-msg="处理">处理</a>
                                            </sys:permission>
