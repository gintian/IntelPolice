...
<div class="bjui-pageHeader">
    <form id="${searchFormId}" data-toggle="ajaxsearch"
          action="${root}<%=Url.getUrl(Url.INVESTIGATIONRELATED_NO_ACCEPT_PAGE_SHOW)%>" method="post">
...
<table class="table table-bordered table-hover table-striped table-top data_table table_center"
    ...
    <c:forEach items="${list}" var="item">
        <tr>
            <td class="option">
                <a href="${root}<%=Url.getUrl(Url.INVESTIGATIONRELATED_DETAIL_SHOW)%>?withBreakId=${item.withBreakId}"
                   class="btn btn-green" data-toggle="dialog" data-mask="true" data-width="800"
                   data-height="600" data-confirm-msg="详情">异常详情</a>
            </td>
            <td title="${item.deptName}"><hs:out value="${item.deptName}" length="10"/></td>
            <td title="${item.suspicTypeName}"><hs:out value="${item.suspicTypeName}" length="15"/></td>
