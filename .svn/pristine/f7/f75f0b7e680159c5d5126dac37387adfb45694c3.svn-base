        ...
        <div class="panel-heading">
            <sys:permission url="<%=Url.getUrl(Url.ARCHIVE_ADD_SHOW)%>">
                <a href="${root}<%=Url.getUrl(Url.ARCHIVE_ADD_SHOW)%>?caseid=${criminalBean.caseid}"
                   class="btn btn-green"  data-toggle="dialog" data-mask="true" data-width="600" data-height="500" data-options="{onClose:function(){opener.location.reload()}}">添加</a>
            </sys:permission>
        </div>
        <!-- Table -->
                    ...
                    <td>
                        <sys:permission url="<%=Url.getUrl(Url.ARCHIVE_DETAIL_SHOW)%>">
                            <a href="${root}<%=Url.getUrl(Url.ARCHIVE_DETAIL_SHOW)%>?id=${archive.id}"
                               class="btn btn-green"
                               data-toggle="dialog" data-mask="true" data-width="600" data-height="500" data-id="archive_detail">详情</a>
                        </sys:permission>
                        <sys:permission url="<%=Url.getUrl(Url.ARCHIVE_UPDATE_SHOW)%>">
                            <a href="${root}<%=Url.getUrl(Url.ARCHIVE_UPDATE_SHOW)%>?id=${archive.id}&caseid=${criminalBean.caseid}"
                               class="btn btn-blue"
                               data-toggle="dialog" data-mask="true" data-width="600" data-height="500" data-title="编辑"  data-options="{onClose:function(){opener.location.reload()}}">编辑</a>
                        </sys:permission><br>
