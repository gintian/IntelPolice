...
<script type="text/javascript">
    $(document).ready(function () {
        ...
        BJUI.ajax('doajax', {
            async: true,
            url: '${root}<%=Url.getUrl(Url.INVOLVED_TYPES_FIND_ALL)%>',
            loadingmask: true,
    ...
    function getUrl(treeId, treeNode) {
        return "${root}<%=Url.getUrl(Url.INVOLVED_TYPES_SHOW_BY_PARENTID)%>?parentId=" + treeNode.id;
    }
...
<div style="float:left; width:150px;margin-right:4px;overflow: auto;">
    <fieldset style="height:100%;">
        <ul id="${treeId}" class="ztree" data-toggle="ztree" style="height: 500px">
        </ul>
    </fieldset>
</div>