                ...
                <div id="layout-01" style="height:94%; overflow:hidden;">
                    <div class="bjui-pageHeader">
                        <form id="${searchFormId}" data-toggle="ajaxsearch"
                              action="${root}<%=Url.getUrl(Url.INVOLVED_DISPOSE_NO_FINISH_SHOW)%>" method="post">
                    ...
                    <table class="table table-bordered table-hover table-striped table-top data_table table_center"
                        ...
                        <c:forEach items="${list}" var="item">
                            <tr>
                                <td class="option">
                                    <c:choose>
                                        <c:when test="${item.taskName=='申请处置'}">
                                            <a href="${root}<%=Url.getUrl(Url.INVOLVED_DISPOSE_TASK_SUBMIT_SHOW)%>?taskId=${item.taskId}&taskName=申请&remark=${item.remark}"
                                               class="btn btn-orange" data-toggle="dialog" data-mask="true" data-width="600"
                                               data-height="500" data-confirm-msg="处理">处理</a>
                                        </c:when>
                                        <c:when test="${item.taskName=='确认处置'}">
                                            <a href="${root}<%=Url.getUrl(Url.INVOLVED_DISPOSE_TASK_SUBMIT_SHOW)%>?taskId=${item.taskId}&taskName=确认&remark=${item.remark}"
                                               class="btn btn-orange" data-toggle="dialog" data-mask="true" data-width="600"
                                               data-height="500" data-confirm-msg="确认处置">确认处置</a>
                                        </c:when>
                                    </c:choose>
